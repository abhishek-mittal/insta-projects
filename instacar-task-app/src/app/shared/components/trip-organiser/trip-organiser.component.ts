...
export class TripOrganiserComponent implements OnInit {
  ...
  ngOnInit() {
    this.initForms();
    console.log('yo')
    if (this._sharedDataJS.tripDetails) {
      this.initForms(this._sharedDataJS.tripDetails);
    }
  }
  ...
  setTripType(type = TRIP_TYPE.one_way) {
    ...
    if (type === TRIP_TYPE.one_way) {
      this._tripForm.controls['returnDate'].setValidators(Validators.required);
    } else {
      this._tripForm.controls['returnDate'].clearValidators();
    }
  ...
  dateChanged(e) {
    ...
    if (this._tripForm.controls['returnDate'].value && (this.fromDate > this._tripForm.controls['returnDate'].value)) {
  ...
  submitJourneyDetails() {
    ...
    const tripDetails = this._tripForm.getRawValue();

    this._sharedDataJS.currentTripDetails(tripDetails)
    this._itS.availableDrivers(tripDetails).subscribe( e => {
      console.log(e);
